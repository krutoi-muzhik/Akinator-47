...
const char *logs_path = "StackLogs.txt";
...
void
StackConstruct (stack_t *stk) { // ToDo: check is created
	stk->canary1 = canary;
	stk->canary2 = canary;
	stk->size = 0;
	stk->capacity = 1;
...
void
StackPush (stack_t *stk, data_t value) {
	int error = StackError (stk);
	if (error)
...
size_t
StackIsEmpty (stack_t *stk) {
	if (GetStackSize (stk) == 0)
		return 1;
	else
		return 0;
}
...
void
Dump (stack_t *stk, int error, const char *function, const int line) {
	switch (error) {
		case NO_ERRORS:
